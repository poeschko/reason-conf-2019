...
**Note:** Please prepare an AWS account setup for the Serverless function exercise!

### Ecosystem (slides/interactive presentation)

- About OCaml and some major projects using it (Jane Street, Docker, Facebook, Mirage, Coq, …)
- Package management. We will lead with Esy for this, since it wraps opam very effectively.
- Common tools: VS Code, Dune, Merlin, utop setup. (10 minutes to get everyone setup)
- Owl

### Language

- Start with the guided tour in RWO chapter 1 (dev.realworldocaml.org) to get users familar with basic syntax. (leave around 20-30 minutes and walk around showing users how things work)
- Discuss common standard frameworks (Lwt and Core/Async). Lwt covers the JavaScript side of things. Won’t cover Containers/Batteries as they’re not as widely used and it muddles things up.

### Practical topics

- Httpaf + websocket server
- Command line tool to query GitHub REST API
- Serverless function using OCaml.
- It’s always fun to end up with a unikernel build. I’ll save this for the “advanced topic” for users that really want to try it.
